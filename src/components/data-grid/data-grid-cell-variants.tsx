...
export function SelectCell<TData>({
  ...
  // Ensure we always work with a string value
  let initialValue = "";
  if (typeof rawValue === "string") {
    initialValue = rawValue;
  } else if (rawValue && typeof rawValue === "object") {
    // Try to extract value from object (for nested structures like {value: "beginner"})
    const objValue = (rawValue as any)?.value;
    if (typeof objValue === "string") {
      initialValue = objValue;
    } else {
      // If it's an object but no value property, it's likely the wrong data structure
      // Log a warning and use empty string
      console.warn("SelectCell received unexpected object value:", rawValue);
      initialValue = "";
    }
  } else {
  ...
  if (initialValue !== prevInitialValueRef.current) {
    ...
    if (typeof initialValue === "string") {
      ...
      console.warn(
        "SelectCell: Attempted to set non-string value:",
        initialValue
      );
      setValue("");
    }
  ...
  // Ensure value is always a string to avoid React rendering errors
  const stringValue = safeValue;
  const matchedOption = options.find((opt) => opt.value === stringValue);
  // Ensure displayLabel is always a string, never an object
  const displayLabel =
